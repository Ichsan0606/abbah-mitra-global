    ...
    <div class="py-6">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4 flex justify-end">
                <a href="{{ route('teams.create') }}" class="px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Tambah Anggota</a>
            </div>
                ...
                <table class="min-w-full divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        @foreach($teams as $team)
                            <tr>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $team->full_name }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ $team->job }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">{{ Str::limit($team->deskripsi, 50) }}</td>
                                <td class="px-6 py-4 whitespace-nowrap">
                                    @if($team->foto)
                                        <img src="{{ asset('storage/'.$team->foto) }}" class="w-15 h-20 rounded">
                                    @endif
                                </td>
